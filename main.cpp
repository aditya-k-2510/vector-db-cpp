...
class KDTree {
    ...
    // canPrune: the pruning test below uses |q[ax] - node[ax]| as a lower
    // bound on the true distance to anything in the far subtree. That bound
    // is valid for Euclidean (per-axis diff <= L2 distance) and Manhattan
    // (per-axis diff <= L1 distance), but NOT for cosine distance, which has
    // no per-axis geometric lower bound. Pruning with cosine silently drops
    // true nearest neighbors. For cosine we visit both subtrees: still exact,
    // but O(N) - which is itself a lesson in why KD-trees don't fit
    // angular metrics (real systems normalize vectors and use L2 instead).
    void knn(KDNode* n, const std::vector<float>& q, int k, int d, DistFn dist,
             std::priority_queue<std::pair<float,int>>& heap, bool canPrune)
    {
        if (!n) return;
        ...
        knn(closer, q, k, d+1, dist, heap, canPrune);
        if (!canPrune || (int)heap.size() < k || std::abs(diff) < heap.top().first)
            knn(farther, q, k, d+1, dist, heap, canPrune);
    }
...
public:
        ...
        const std::string& metric = "euclidean")
    {
        ...
        bool canPrune = (metric != "cosine");   // see comment above
        knn(root, q, k, 0, dist, heap, canPrune);
        std::vector<std::pair<float,int>> r;
...
class HNSW {
    ...
    // Neighbor selection heuristic (Algorithm 4 from the HNSW paper).
    // Naive top-M selection picks the M closest candidates, but those tend
    // to cluster together on one side of the query - the graph loses
    // long-range "shortcut" edges and becomes hard to navigate. The
    // heuristic keeps a candidate only if it is closer to the base point
    // than to every neighbor already selected, which spreads edges across
    // directions and preserves graph navigability (and therefore recall).
    // cands must be sorted ascending by distance to the base point.
    std::vector<int> selectNbrs(const std::vector<float>& base,
                                const std::vector<std::pair<float,int>>& cands,
                                int maxM, DistFn dist)
    {
        std::vector<int> r;
        std::vector<std::pair<float,int>> skipped;
        for (auto& [cd, cid] : cands) {
            if ((int)r.size() >= maxM) break;
            if (!G.count(cid)) continue;
            bool diverse = true;
            for (int sid : r) {
                if (dist(G[cid].item.emb, G[sid].item.emb) < cd) {
                    diverse = false; break;      // cid is "behind" a chosen neighbor
                }
            }
            if (diverse) r.push_back(cid);
            else         skipped.push_back({cd, cid});
        }
        // keepPrunedConnections: backfill with the closest skipped candidates
        // so nodes are not left under-connected.
        for (auto& [cd, cid] : skipped) {
            if ((int)r.size() >= maxM) break;
            r.push_back(cid);
        }
        return r;
...
public:
    ...
    void insert(const VectorItem& item, DistFn dist) {
        ...
        for (int lc = std::min(topLayer, lvl); lc >= 0; lc--) {
            ...
            int maxM = (lc == 0) ? M0 : M;
            auto sel = selectNbrs(item.emb, W, maxM, dist);
            G[id].nbrs[lc] = sel;
            ...
            for (int nid : sel) {
                ...
                if ((int)conn.size() > maxM) {
                    // Re-select this neighbor's edges with the same diversity
                    // heuristic instead of naively keeping the M closest.
                    std::vector<std::pair<float,int>> ds;
                    for (int c : conn) if (G.count(c))
                        ds.push_back({dist(G[nid].item.emb, G[c].item.emb), c});
                    std::sort(ds.begin(), ds.end());
                    conn = selectNbrs(G[nid].item.emb, ds, maxM, dist);
                }
            }
            if (!W.empty()) ep = W[0].second;
        }
...
// Thread-safety model: cpp-httplib serves each request on a thread-pool
// thread, so this class is hit concurrently. None of the index structures
// (HNSW graph, KD-tree, BruteForce vector) are internally synchronized,
// so synchronization happens here, at the database boundary.
//
// A plain std::mutex would be *correct* but fully serializes the server:
// two searches cannot run at the same time even though neither mutates
// anything. std::shared_mutex gives reader-writer semantics instead -
// searches take a shared lock (many can run concurrently), while
// insert/remove take a unique lock (exclusive, waits for readers to
// drain). Since a vector DB workload is overwhelmingly read-heavy, this
// turns lock contention from "every request queues" into "only writes
// block".
class VectorDB {
...
public:
    ...
    {
        std::unique_lock<std::shared_mutex> lk(mu);   // writer: exclusive
...
class DocumentDB {
    ...
    mutable std::shared_mutex mu;   // same reader-writer model as VectorDB
    int nextId = 1;
...
public:
    ...
    {
        std::unique_lock<std::shared_mutex> lk(mu);   // writer
    ...
    {
        std::shared_lock<std::shared_mutex> lk(mu);   // reader
...
// =====================================================================
//  SCALE BENCHMARK  (./db --bench [N] [dims])
//
//  A benchmark on 20 vectors proves nothing - brute force wins at that
//  size. This mode builds indexes over N clustered random vectors and
//  measures what actually matters for an ANN index:
//    1. Query latency of BruteForce vs KD-tree vs HNSW as N grows
//       (where is the crossover point?)
//    2. Recall@k: HNSW is *approximate* - what fraction of the true
//       k nearest neighbors (brute-force ground truth) does it return?
//    3. The ef knob: sweeping ef shows the recall/latency tradeoff that
//       every production vector DB exposes to its users.
// =====================================================================

// Clustered Gaussian data - more realistic than uniform noise, because
// real embeddings cluster by semantics (that's the entire point of them).
static std::vector<std::vector<float>> genClustered(int n, int dims,
                                                    int clusters, std::mt19937& rng)
{
    std::normal_distribution<float>       noise(0.0f, 0.15f);
    std::uniform_real_distribution<float> center(-1.0f, 1.0f);
    std::vector<std::vector<float>> centers(clusters, std::vector<float>(dims));
    for (auto& c : centers) for (auto& x : c) x = center(rng);

    std::vector<std::vector<float>> out(n, std::vector<float>(dims));
    for (int i = 0; i < n; i++) {
        auto& c = centers[i % clusters];
        for (int d = 0; d < dims; d++) out[i][d] = c[d] + noise(rng);
    }
    return out;
}

static float recallAtK(const std::vector<std::pair<float,int>>& approx,
                       const std::vector<std::pair<float,int>>& truth)
{
    if (truth.empty()) return 1.0f;
    std::set<int> t;
    for (auto& [d, id] : truth) t.insert(id);
    int hit = 0;
    for (auto& [d, id] : approx) if (t.count(id)) hit++;
    return (float)hit / (float)truth.size();
}

static void runBench(int N, int dims) {
    const int Q = 50, K = 10;
    std::mt19937 rng(7);
    auto dfn = getDistFn("euclidean");

    std::cout << "\n=== N=" << N << "  dims=" << dims
              << "  (" << Q << " queries, k=" << K << ") ===\n";

    // Generate data and queries from the SAME distribution (same cluster
    // centers) - the last Q points become queries. Querying from a
    // different distribution than the data makes recall numbers
    // meaninglessly pessimistic.
    auto allPts  = genClustered(N + Q, dims, 32, rng);
    std::vector<std::vector<float>> data(allPts.begin(), allPts.begin() + N);
    std::vector<std::vector<float>> queries(allPts.begin() + N, allPts.end());
    ...
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < N; i++) {
        VectorItem v{i + 1, "", "", data[i]};
        bf.insert(v); kdt.insert(v); hnsw.insert(v, dfn);
    }
    auto buildMs = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::high_resolution_clock::now() - t0).count();
    std::cout << "build (all 3 indexes): " << buildMs << " ms\n";

    // Ground truth from brute force (exact by definition)
    std::vector<std::vector<std::pair<float,int>>> truth(Q);
    for (int q = 0; q < Q; q++) truth[q] = bf.knn(queries[q], K, dfn);

    auto avgUs = [&](auto fn) -> double {
        auto t = std::chrono::high_resolution_clock::now();
        for (int q = 0; q < Q; q++) fn(q);
        return std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::high_resolution_clock::now() - t).count() / (double)Q;
    };

    double bfUs = avgUs([&](int q){ bf.knn(queries[q], K, dfn); });
    double kdUs = avgUs([&](int q){ kdt.knn(queries[q], K, dfn, "euclidean"); });

    float kdRec = 0;
    for (int q = 0; q < Q; q++)
        kdRec += recallAtK(kdt.knn(queries[q], K, dfn, "euclidean"), truth[q]);
    kdRec /= Q;

    printf("%-22s %10.1f us/query   recall@%d = %.3f\n", "BruteForce (exact)", bfUs, K, 1.0f);
    printf("%-22s %10.1f us/query   recall@%d = %.3f\n", "KD-Tree    (exact)", kdUs, K, kdRec);

    // ef sweep: THE approximate-search tradeoff
    for (int ef : {10, 20, 50, 100, 200}) {
        float rec = 0;
        for (int q = 0; q < Q; q++)
            rec += recallAtK(hnsw.knn(queries[q], K, ef, dfn), truth[q]);
        rec /= Q;
        double us = avgUs([&](int q){ hnsw.knn(queries[q], K, ef, dfn); });
        printf("HNSW  ef=%-4d          %10.1f us/query   recall@%d = %.3f   (%.0fx vs brute force)\n",
               ef, us, K, rec, bfUs / std::max(us, 0.1));
    }
}
...
int main(int argc, char** argv) {
    if (argc >= 2 && std::string(argv[1]) == "--bench") {
        std::cout << "VectorDB scale benchmark\n";
        if (argc >= 3) {
            // custom run: ./db --bench N [dims]
            int n = std::stoi(argv[2]);
            int d = (argc >= 4) ? std::stoi(argv[3]) : 16;
            runBench(n, d);
        } else {
            // default suite: crossover sweep at 16D, then a high-dim case
            // to demonstrate the curse of dimensionality killing KD-tree
            for (int n : {1000, 10000, 50000}) runBench(n, 16);
            runBench(20000, 128);
        }
        return 0;
    }
